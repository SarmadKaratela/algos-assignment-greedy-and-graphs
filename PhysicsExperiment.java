 ...
 */
 ...
 public int[][] scheduleExperiments(
  ...
  int CurrentStep = 1;
  int RemainStep= numSteps;

    while (RemainStep > 0){

     if(signUpTable[CurrentStudent][CurrentStep] == 1)
     {
      ...
      else
      {
        if(CurrentStep != numSteps){
           int consecutiveStudents = 0; //count that will track the most consecutive steps a student will do in a row. 
           int index = 1; //index to keep track of those consecutive steps
              //to get the optimal solution we need to check the most consecutive steps that a stuend can do
           for (int i=1; i<=numStudents;i++){
             //finding the current number of the most consecutive steps a student can do
            ...
            for(int j=CurrentStep; j<= numSteps; j++){
              ...
              if (maxConsecutiveSteps > consecutiveStudents){
                consecutiveStudents = maxConsecutiveSteps;
                index = i;
                }
              }
            }
